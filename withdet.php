...
<nav>
	...
	$account=$row['0'];
	$sql="Select * from withdraw where AccountNo='$account'";
	$result=$conn->query($sql);
	if($result-> num_rows>0){
		while($row= $result->fetch_assoc()) {
			echo"<tr><td align=center>". $row["AccountNo"] ."</td><td align=center>". $row["WithdrawName"] ."</td><td align=center>". $row["Amount"] ."</td></tr>";
		}
		echo"</table>";
	}
	else{
